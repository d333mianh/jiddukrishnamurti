...
### The I Ching navigation layer

An **additional** way into the same 36 roots, added 2026-07-25. Eight trigram
"gates" pair up as `8 self-pairs + 28 distinct pairs = 36 bridges` — the number
36 is *derived* from the structure, not assigned to fit. Each unordered gate
pair maps to exactly one root; each pair also renders as two hexagram figures
(lower/upper swapped), both shown, neither canonical.

- Data: `concepts/iching_navigation.json` + `scripts/iching_data.py` (King Wen
  table, Unicode glyphs `U+4DC0–U+4DFF`, cast decoding, validation).
- Vault: `obsidian/roots/I Ching Navigator.md` + `obsidian/roots/gates/*.md`,
  generated by `scripts/build_concept_vault.py`.
- **Navigator, not oracle.** A cast selects *what to inquire into*. It never
  answers, predicts, or advises, and carries no traditional judgment or line
  texts — those are copyright-encumbered and would restore exactly the authority
  K spent his life dismantling.
- It is **navigation only**: not part of any root's definition. `concepts.jsonl`
  stays canonical, and the layer is marked `status: provisional`.

Grounding: K discusses the I Ching in three archived conversations with Allan W.
Anderson, who each time quotes it to test a translation of what K had just said —
`SD72CA1`, `SD74CA4`, `SD74CA5`. Those map to the roots *listening*,
*responsibility*, and *order*. A coincidence worth noticing, not evidence of
anything.
...
**Vault (L4)** — 46 generated notes (36 concepts + Map + Navigator + 8 gates).
