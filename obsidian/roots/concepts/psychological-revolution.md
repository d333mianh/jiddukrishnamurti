---
tags: [krishnamurti, root, concept, status/active]
slug: psychological-revolution
facet: "IV. Ending, Transformation & the Sacred"
status: active
aliases: ["Transformation & Mutation", "transformation", "mutation", "psychological revolution", "the only revolution", "regeneration"]
---
# Transformation & Mutation

> [!note] Active

**Facet:** IV. Ending, Transformation & the Sacred · **Root 35 of 36** · [[Map of the 36 Roots|↩ Map]] · [[Strategy]]

## Definition
Krishnamurti calls for a radical transformation — total and immediate, not the gradual reform of becoming — that alone can end humanity's inherited disorder. In his earlier decades he framed this as the 'only revolution', which is inward; in the later dialogues he spoke of a mutation of the mind and even of the brain cells (his own formulation, not a neurological claim).

## What counts as this root
Include passages examining radical inward transformation, mutation, regeneration, the 'only revolution', or immediate change versus gradual becoming.

## What does not
Exclude political, social, or economic revolution unless explicitly contrasted with inward transformation.

## Also called
- transformation
- mutation _(brain-cell phrasing prominent in later talks and Bohm dialogues)_
- psychological revolution _(earlier-decade phrasing)_
- the only revolution
- regeneration

## Related roots
- [[psychological-time|Psychological Time & Becoming]] · *contrasts_with* — Mutation is immediate; becoming is gradual.
- [[insight|Insight & Intelligence]] · *related* — Insight is the instrument of mutation.

---
*Generated from `concepts/concepts.jsonl` by `scripts/build_concept_vault.py`. Edit the JSONL, then regenerate — do not hand-edit this note.*
